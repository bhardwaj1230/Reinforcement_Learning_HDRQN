...
class Coach:
    def __init__(self):
        ...
        self.collect_intrinsic_reward= []
        self.collect_ale_lives = []
        self.collect_agent_death = []
        self.collect_extrinsic_reward_for_goal = []
        self.collect_actor_epsilon = []
        self.collect_success_rate = []
        self.collect_annealing_schedule = []
        self.collect_annealing_factor_per_episode = []
    ...
    def learn_subgoal(self,num_episode):
        ...
        np.savez('./learn_subgoal.npz', collect_intrinsic_reward = self.collect_intrinsic_reward, collect_ale_lives = self.collect_ale_lives, collect_agent_death = self.collect_agent_death, collect_extrinsic_reward_for_goal = self.collect_extrinsic_reward_for_goal)                
    ...
    def learn_global(self):
        ...
        for num_episode in tqdm(range(self.num_episodes)):
                ...
                self.collect_annealing_schedule.append(self.annealing_schedule())
                for goal in self.goal_idx.keys():
                    ...
                    if self.stats['goal_selected'][self.goal_idx[goal]] > 0:
                        ...
                        self.collect_success_rate.append((self.stats['goal_success'][self.goal_idx[goal]], self.stats['goal_selected'][self.goal_idx[goal]], goal))
                        ...
                        self.collect_actor_epsilon.append((goal, self.agent.actor_epsilon[self.goal_idx[goal]]))
        return self.collect_actor_epsilon, self.collect_success_rate, self.collect_annealing_schedule, self.collect_annealing_factor_per_episode
        

def main():
    coach = Coach()
    #coach.learn_global()
    collect_actor_epsilon, collect_success_rate , collect_annealing_schedule, collect_annealing_factor_per_episode = coach.learn_global()
    np.savez('./learn_global.npz', collect_actor_epsilon = collect_actor_epsilon, collect_success_rate = collect_success_rate, collect_annealing_schedule = collect_annealing_schedule, collect_annealing_factor_per_episode = collect_annealing_factor_per_episode)
