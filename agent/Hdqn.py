...
class Hdqn:
	...
	def select_move(self, state, goal, goal_value):
		...
		# print input_vector.shape
		action_prob = self.actor(Variable(torch.from_numpy(input_vector).type(dtype), requires_grad = False)).data

		# print("Action_pro", action_prob)
		# raw_input()
		#print(action_prob)
		#return np.argmax(action_prob)
		#print('\n\n\n\n', action_prob.max().cpu().detach().numpy(), '\n\n\n\n')
		return  np.argmax(action_prob.cpu().detach().numpy())
	...
	def update(self):
		...
		self.collect_loss.append(loss.data)
		...
		np.savez('./loss.npz', collect_loss = self.collect_loss)